...
load = 0
loadfile = 'Signal_1500_20W_7-sae.h5'
savefile = 'Signal_1500_20W_7-sae.h5'
dataset = 'Signal_1500_20W_7.mat'
...
def Net_model(lr=0.001,decay=1e-6,momentum=0.9):
    model = Sequential()
    model.add(Dense(2000, input_dim=1500,W_regularizer=l2(0.001)))
    model.add(Activation(ELU(alpha=1.0)))
    model.add(BN(epsilon=1e-06, mode=0, axis=1, momentum=momentum))    
    model.add(Dropout(0.2))
    
    model.add(Dense(1500,W_regularizer=l2(0.001))) #Full connection
    model.add(Activation(ELU(alpha=1.0)))
    model.add(BN(epsilon=1e-06, mode=0, axis=1, momentum=momentum))    
    model.add(Dropout(0.2))     
    
    model.add(Dense(8000,W_regularizer=l2(0.001))) #Full connection
    model.add(Activation(ELU(alpha=1.0)))
    model.add(BN(epsilon=1e-06, mode=0, axis=1, momentum=momentum))
    model.add(Dropout(0.2))  

    model.add(Dense(300,W_regularizer=l2(0.001))) #Full connection
    model.add(Activation(ELU(alpha=1.0)))
    model.add(BN(epsilon=1e-06, mode=0, axis=1, momentum=momentum))
    model.add(Dropout(0.2))
